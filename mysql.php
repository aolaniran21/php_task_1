...
function
remove_slashes($Ye, $tc = false)
{
	if (get_magic_quotes_gpc()) {
		while (list($z, $X) = each($Ye)) {
			foreach ($X
				as $fd => $W) {
				unset($Ye[$z][$fd]);
				if (is_array($W)) {
				...
				} else $Ye[$z][stripslashes($fd)] = ($tc ? $W : stripslashes($W));
			}
		}
	}
}
function
bracket_escape($Qc, $_a = false)
{
	static $Eg = array(':' => ':1', ']' => ':2', '[' => ':3', '"' => ':4');
	return
		strtr($Qc, ($_a ? array_flip($Eg) : $Eg));
}
function
min_version($eh, $zd = "", $f = null)
{
	global $e;
	if (!$f) $f = $e;
	$Gf = $f->server_info;
...
function
optionlist($me, $Bf = null, $Yg = false)
{
	$K = "";
	foreach ($me
		as $fd => $W) {
		...
		foreach ($ne
			as $z => $X) $K .= '<option' . ($Yg || is_string($z) ? ' value="' . h($z) . '"' : '') . (($Yg || is_string($z) ? (string)$z : $X) === $Bf ? ' selected' : '') . '>' . h($X);
		if (is_array($W)) $K .= '</optgroup>';
	}
	return $K;
}
function
html_select($E, $me, $Y = "", $he = true, $kd = "")
{
	if ($he) return "<select name='" . h($E) . "'" . ($kd ? " aria-labelledby='$kd'" : "") . ">" . optionlist($me, $Y) . "</select>" . (is_string($he) ? script("qsl('select').onchange = function () { $he };", "") : "");
	$K = "";
	foreach ($me
...
{
	$_SESSION["pwds"][$dh][$O][$V] = ($_COOKIE["adminer_key"] && is_string($G) ? array(encrypt_string($G, $_COOKIE["adminer_key"])) : $G);
}
function
get_password()
{
...
function
get_key_vals($I, $f = null, $ug = 0, $Jf = true)
{
	global $e;
	if (!is_object($f)) $f = $e;
	$K = array();
...
function
query_redirect($I, $B, $D, $gf = true, $gc = true, $nc = false, $tg = "")
{
	global $e, $k, $b;
	if ($gc) {
		$Tf = microtime(true);
	...
	$Pf = "";
	if ($I) $Pf = $b->messageQuery($I, $tg, $nc);
	if ($nc) {
		$k = error() . $Pf . script("messagesPrint();");
		return
			false;
	}
	if ($gf) redirect($B, $D . $Pf);
	return
		true;
}
function
...
function
apply_queries($I, $S, $cc = 'table')
{
	foreach ($S
		as $Q) {
		if (!queries("$I " . $cc($Q))) return
			false;
	}
	return
		true;
...
function
queries_redirect($B, $D, $gf)
{
	list($bf, $tg) = queries(null);
	return
...
get_file($z, $xb = false)
{
	...
	foreach ($rc["error"] as $z => $k) {
		...
		if ($xb) {
			$Tf = substr($fb, 0, 3);
			if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $Tf, $mf)) $fb = iconv("utf-16", "utf-8", $fb);
			elseif ($Tf == "\xEF\xBB\xBF") $fb = substr($fb, 3);
			$K .= $fb . "\n\n";
		} else $K .= $fb;
	}
	return $K;
...
function
shorten_utf8($Xf, $sd = 80, $bg = "")
{
	if (!preg_match("(^(" . repeat_pattern("[\t\r\n -\x{10FFFF}]", $sd) . ")($)?)u", $Xf, $C)) preg_match("(^(" . repeat_pattern("[\t\r\n -~]", $sd) . ")($)?)", $Xf, $C);
	return
...
input($l, $Y, $q)
{
	...
	if ($of && !$_POST["save"]) $q = null;
	$Bc = (isset($_GET["select"]) || $of ? array("orig" => 'original') : array()) + $b->editFunctions($l);
	$wa = " name='fields[$E]'";
	if ($l["type"] == "enum") echo
	nbsp($Bc[""]) . "<td>" . $b->editInput($_GET["edit"], $l, $wa, $Y);
		...
		elseif (($rg = preg_match('~text|lob~', $l["type"])) || preg_match("~\n~", $Y)) {
			if ($rg && $y != "sqlite") $wa .= " cols='50' rows='12'";
			else {
			...
			if ($y == 'sql' && min_version(5.6) && preg_match('~time~', $l["type"])) $Gd += 7;
...
process_input($l)
{
	...
	if ($l["auto_increment"] && $Y == "") return
		null;
	if ($q == "orig") return ($l["on_update"] == "CURRENT_TIMESTAMP" ? idf_escape($l["field"]) : false);
...
function
dump_headers($Pc, $Nd = false)
{
	global $b;
	$K = $b->dumpHeaders($Pc, $Nd);
	$ye = $_POST["output"];
	if ($ye != "text") header("Content-Disposition: attachment; filename=" . $b->dumpFilename($Pc) . ".$K" . ($ye != "file" && !preg_match('~[^0-9a-z]~', $ye) ? ".$ye" : ""));
	session_write_close();
	ob_flush();
	flush();
	return $K;
...
function
select_value($X, $A, $l, $sg)
{
	global $b;
	if (is_array($X)) {
		$K = "";
		foreach ($X
			as $fd => $W) $K .= "<tr>" . ($X != array_values($X) ? "<th>" . h($fd) : "") . "<td>" . select_value($W, $A, $l, $sg);
	...
	if ($K !== null) {
		if ($K === "") $K = "&nbsp;";
		elseif (!is_utf8($K)) $K = "\0";
		elseif ($sg != "" && is_shortable($l)) $K = shorten_utf8($K, max(0, +$sg));
		else $K = h($K);
	}
...
edit_form($a, $m, $L, $Tg)
{
	...
	page_header(($Tg ? 'Edit' : 'Insert'), $k, array("select" => array($a, $gg)), $gg);
...
';
	if (!$m) echo "<p class='error'>" . 'You have no privileges to update this table.' . "\n";
		...
		foreach ($m
			...
			if ($yb === null) {
				$yb = $l["default"];
				if ($l["type"] == "bit" && preg_match("~^b'([01]*)'\$~", $yb, $mf)) $yb = $mf[1];
			}
...
if (!defined("SID")) {
	...
	$Be = array(0, preg_replace('~\\?.*~', '', $_SERVER["REQUEST_URI"]), "", $ba);
	if (version_compare(PHP_VERSION, '5.2.0') >= 0) $Be[] = true;
	call_user_func_array('session_set_cookie_params', $Be);
	session_start();
}
remove_slashes(array(&$_GET, &$_POST, &$_COOKIE), $tc);
...
if (extension_loaded('pdo')) {
	...
	PDO
	{
		...
		__construct()
		{
			...
			if ($Ne !== false) unset($b->operators[$Ne]);
		}
		function
		dsn($Mb, $V, $G, $me = array())
		{
...
Min_SQL
{
	...
	function
	select($Q, $N, $Z, $s, $oe = array(), $_ = 1, $F = 0, $Ue = false)
	{
		global $b, $y;
		$cd = (count($s) < count($N));
		$I = $b->selectQueryBuild($N, $Z, $s, $oe, $_, $F);
		if (!$I) $I = "SELECT" . limit(($_GET["page"] != "last" && $_ != "" && $s && $cd && $y == "sql" ? "SQL_CALC_FOUND_ROWS " : "") . implode(", ", $N) . "\nFROM " . table($Q), ($Z ? "\nWHERE " . implode(" AND ", $Z) : "") . ($s && $cd ? "\nGROUP BY " . implode(", ", $s) : "") . ($oe ? "\nORDER BY " . implode(", ", $oe) : ""), ($_ != "" ? +$_ : null), ($F ? $_ * $F : 0), "\n");
		$Tf = microtime(true);
		$K = $this->_conn->query($I);
		if ($Ue) echo $b->selectQuery($I, $Tf, !$K);
		return $K;
	}
	function
	delete($Q, $cf, $_ = 0)
...
$Ib = array("server" => "MySQL") + $Ib;
if (!defined("DRIVER")) {
	$Qe = array("MySQLi", "MySQL", "PDO_MySQL");
	...
	if (extension_loaded("mysqli")) {
		...
		MySQLi
		{
			...
			function
			connect($O = "", $V = "", $G = "", $tb = null, $Me = null, $Mf = null)
			{
				global $b;
				mysqli_report(MYSQLI_REPORT_OFF);
				list($Nc, $Me) = explode(":", $O, 2);
				$Sf = $b->connectSsl();
				if ($Sf) $this->ssl_set($Sf['key'], $Sf['cert'], $Sf['ca'], '', '');
		...
		Min_PDO
		{
			...
			connect($O, $V, $G)
			{
				...
				$me = array();
				$Sf = $b->connectSsl();
				if ($Sf) $me = array(PDO::MYSQL_ATTR_SSL_KEY => $Sf['key'], PDO::MYSQL_ATTR_SSL_CERT => $Sf['cert'], PDO::MYSQL_ATTR_SSL_CA => $Sf['ca'],);
	...
	Min_SQL
	{
		...
		tableHelp($E)
		{
			...
			if (DB == "mysql") return ($yd ? "mysql$E-table/" : "system-database.html");
		}
	}
	function
	idf_escape($Qc)
	...
	connect()
	{
		...
		$K = $e->error;
		if (function_exists('iconv') && !is_utf8($K) && strlen($xf = iconv("windows-1250", "utf-8", $K)) > strlen($K)) $K = $xf;
		return $K;
	}
	function
	get_databases($vc)
	...
	foreign_keys($Q)
	{
		...
		if ($kb) {
			...
			foreach ($_d
				...
				$K[idf_unescape($C[1])] = array("db" => idf_unescape($C[4] != "" ? $C[3] : $C[4]), "table" => idf_unescape($C[4] != "" ? $C[4] : $C[3]), "source" => array_map('idf_unescape', $Nf[0]), "target" => array_map('idf_unescape', $ng[0]), "on_delete" => ($C[6] ? $C[6] : "RESTRICT"), "on_update" => ($C[7] ? $C[7] : "RESTRICT"),);
			}
		}
		return $K;
	}
	...
	rename_database($E, $Wa)
	{
		...
		if (create_database($E, $Wa)) {
			$nf = array();
			foreach (tables_list() as $Q => $U) $nf[] = table($Q) . " TO " . idf_escape($E) . "." . table($Q);
	...
	function
	alter_table($Q, $E, $m, $wc, $bb, $Xb, $Wa, $ya, $Fe)
	{
		$sa = array();
		foreach ($m
	...
	{
		$nf = array();
		foreach (array_merge($S, $gh) as $Q) $nf[] = table($Q) . " TO " . idf_escape($ng) . "." . table($Q);
		return
	...
	copy_tables($S, $gh, $ng)
	{
		...
		foreach ($gh
			...
			$E = ($ng == DB ? table("copy_$Q") : idf_escape($ng) . "." . table($Q));
			$fh = view($Q);
			if (!queries("DROP VIEW IF EXISTS $E") || !queries("CREATE VIEW $E AS $fh[select]")) return
				false;
		}
		return
			true;
	...
	routine($E, $U)
	{
		...
		foreach ($_d
			...
			$m[] = array("field" => $E, "type" => strtolower($Ae[5]), "length" => preg_replace_callback("~$Zb~s", 'normalize_enum', $Ae[6]), "unsigned" => strtolower(preg_replace('~\\s+~', ' ', trim("$Ae[8] $Ae[7]"))), "null" => 1, "full_type" => $Ae[4], "inout" => strtoupper($Ae[1]), "collation" => strtolower($Ae[9]),);
		}
	...
	unconvert_field($l, $K)
	{
		...
		if (preg_match("~geometry|point|linestring|polygon~", $l["type"])) $K = (min_version(8) ? "ST_" : "") . "GeomFromText($K)";
		return $K;
	}
	function
	support($pc)
	...
	$Sg = array("unsigned", "zerofill", "unsigned zerofill");
	$ke = array("=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "REGEXP", "IN", "FIND_IN_SET", "IS NULL", "NOT LIKE", "NOT REGEXP", "NOT IN", "IS NOT NULL", "SQL");
	$Bc = array("char_length", "date", "from_unixtime", "lower", "round", "floor", "ceil", "sec_to_time", "time_to_sec", "upper");
	$Fc = array("avg", "count", "count distinct", "group_concat", "max", "min", "sum");
...
Adminer
{
	...
	loginForm()
	{
		...
		<table cellspacing="0">
			<tr>
				<th>System
				<td><input type='hidden' name='auth[driver]' value='server'>MySQL
			<tr>
				<th>Server
				<td><input name="auth[server]" value="<?php echo
...
', script("focus(qs('#username'));"), "<p><input type='submit' value='" . 'Login' . "'>\n", checkbox("auth[permanent]", 1, $_COOKIE["adminer_permanent"], 'Permanent login') . "\n";
													...
													selectLinks($fg, $P = "")
													{
														...
														if (support("table") || support("indexes")) $vd["table"] = 'Show structure';
														if (support("table")) {
															if (is_view($fg)) $vd["view"] = 'Alter view';
															else $vd["create"] = 'Alter table';
														}
														if ($P !== null) $vd["edit"] = 'New item';
														$E = $fg["Name"];
														foreach ($vd
													...
													selectVal($X, $A, $l, $we)
													{
														...
														if (preg_match('~blob|bytea|raw|file~', $l["type"]) && !is_utf8($X)) $K = "<i>" . lang(array('%d byte', '%d bytes'), strlen($we)) . "</i>";
													...
													tableIndexesPrint($w)
													{
														...
														foreach ($w
															...
															ksort($v["columns"]);
															$Ue = array();
															foreach ($v["columns"] as $z => $X) $Ue[] = "<i>" . h($X) . "</i>" . ($v["lengths"][$z] ? "(" . $v["lengths"][$z] . ")" : "") . ($v["descs"][$z] ? " DESC" : "");
													...
													selectColumnsPrint($N, $d)
													{
														...
														foreach ($N
															...
															echo "<div>" . ($Bc || $Fc ? "<select name='columns[$t][fun]'>" . optionlist(array(-1 => "") + array_filter(array('Functions' => $Bc, 'Aggregation' => $Fc)), $X["fun"]) . "</select>" . on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'", 1) . script("qsl('select').onchange = function () { helpClose();" . ($z !== "" ? "" : " qsl('select, input', this.parentNode).onchange();") . " };", "") . "($c)" : $c) . "</div>\n";
															$t++;
														}
														echo "</div></fieldset>\n";
													}
													...
													selectSearchProcess($m, $w)
													{
														...
														foreach ((array)$_GET["where"] as $z => $X) {
															if ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators)) {
																...
																if ($X["col"] != "") $K[] = $Re . $j->convertSearch(idf_escape($X["col"]), $X, $m[$X["col"]]) . $db;
																else {
																	$Ya = array();
																	foreach ($m
																		as $E => $l) {
																		if ((is_numeric($X["val"]) || !preg_match('~' . number_type() . '|bit~', $l["type"])) && (!preg_match("~[\x80-\xFF]~", $X["val"]) || preg_match('~char|text|enum|set~', $l["type"]))) $Ya[] = $Re . $j->convertSearch(idf_escape($E), $X, $l) . $db;
																	}
													...
													function
													selectEmailProcess($Z, $xc)
													{
														return
															false;
													}
													function
													selectQueryBuild($N, $Z, $s, $oe, $_, $F)
													{
														return "";
													}
													function
													messageQuery($I, $tg, $nc = false)
													{
														global $y, $j;
														restart_session();
														$Lc = &get_session("queries");
														if (!$Lc[$_GET["db"]]) $Lc[$_GET["db"]] = array();
														if (strlen($I) > 1e6) $I = preg_replace('~[\x80-\xFF]+$~', '', substr($I, 0, 1e6)) . "\n...";
														$Lc[$_GET["db"]][] = array($I, time(), $tg);
													...
													editFunctions($l)
													{
														...
														foreach ($Pb
															...
															if (!$z || (!isset($_GET["call"]) && (isset($_GET["select"]) || where($_GET)))) {
																foreach ($Bc
																	as $Je => $X) {
																	if (!$Je || preg_match("~$Je~", $l["type"])) $K .= "/$X";
																}
													...
													dumpTable($Q, $Zf, $ed = 0)
													{
														if ($_POST["format"] != "sql") {
															echo "\xef\xbb\xbf";
															if ($Zf) dump_csv(array_keys(fields($Q)));
														} else {
															if ($ed == 2) {
																$m = array();
																foreach (fields($Q) as $E => $l) $m[] = idf_escape($E) . " $l[full_type]";
																$g = "CREATE TABLE " . table($Q) . " (" . implode(", ", $m) . ")";
															} else $g = create_sql($Q, $_POST["auto_increment"], $Zf);
															set_utf8mb4($g);
															if ($Zf && $g) {
																if ($Zf == "DROP+CREATE" || $ed == 1) echo "DROP " . ($ed == 2 ? "VIEW" : "TABLE") . " IF EXISTS " . table($Q) . ";\n";
																if ($ed == 1) $g = remove_definer($g);
																echo "$g;\n\n";
															}
														}
													}
															...
															if ($J) {
																...
																while ($L = $J->$qc()) {
																	...
																	if ($_POST["format"] != "sql") {
																		...
																		if (!$Xc) $Xc = "INSERT INTO " . table($Q) . " (" . implode(", ", array_map('idf_escape', $gd)) . ") VALUES";
																		foreach ($L
																			as $z => $X) {
																			$l = $m[$z];
																		...
																		$xf = ($Bd ? "\n" : " ") . "(" . implode(",\t", $L) . ")";
																		if (!$Ia) $Ia = $Xc . $xf;
																		elseif (strlen($Ia) + 4 + strlen($xf) + strlen($bg) < $Bd) $Ia .= ",$xf";
																		else {
																			echo $Ia . $bg;
																			$Ia = $Xc . $xf;
																		}
													...
													dumpHeaders($Pc, $Nd = false)
													{
														...
														if ($ye == "gz") ob_start('ob_gzencode', 1e6);
														return $kc;
													}
													function
													importServerPath()
																...
																foreach ($Hf
																	...
																	foreach ($Zg
																		...
																		if ($G !== null) {
																			...
																			foreach (($wb ? array_keys($wb) : array("")) as $i) echo "<a href='" . h(auth_url($dh, $O, $V, $i)) . "'>($Ib[$dh]) " . h($V . ($O != "" ? "@" . $this->serverName($O) : "") . ($i != "" ? " - $i" : "")) . "</a><br>\n";
																		}
																	}
																}
															}
...
';
														...
														echo "<span title='" . 'database' . "'>" . 'DB' . "</span>: " . ($h ? "<select name='db'>" . optionlist(array("" => "") + $h, DB) . "</select>$ub" : "<input name='db' value='" . h(DB) . "' autocapitalize='off'>\n"), "<input type='submit' value='" . 'Use' . "'" . ($h ? " class='hidden'" : "") . ">\n";
													...
													tablesPrint($S)
													{
														...
														foreach ($S
															...
															$E = $this->tableName($Uf);
															if ($E != "") {
																echo '<li><a href="' . h(ME) . 'select=' . urlencode($Q) . '"' . bold($_GET["select"] == $Q || $_GET["edit"] == $Q, "select") . ">" . 'select' . "</a> ", (support("table") || support("indexes") ? '<a href="' . h(ME) . 'table=' . urlencode($Q) . '"' . bold(in_array($Q, array($_GET["table"], $_GET["create"], $_GET["indexes"], $_GET["foreign"], $_GET["trigger"])), (is_view($Uf) ? "view" : "structure")) . " title='" . 'Show structure' . "'>$E</a>" : "<span>$E</span>") . "\n";
															}
														}
														echo "</ul>\n";
													}
												...
												$b = (function_exists('adminer_object') ? adminer_object() : new
													Adminer);
												if ($b->operators === null) $b->operators = $ke;
												function
												page_header($wg, $k = "", $Ha = array(), $xg = "")
												{
													global $ca, $ga, $b, $Ib, $y;
													page_headers();
													if (is_ajax() && $k) {
...
-----END PUBLIC KEY-----
";
														if (openssl_verify($eh["version"], base64_decode($eh["signature"]), $af) == 1) $_COOKIE["adminer_version"] = $eh["version"];
													}
													echo '<script', nonce(), '>
...
';
														...
														if ($Ha === false) echo "$O\n";
															...
															if (is_array($Ha)) {
																...
																foreach ($Ha
																	...
																	if ($Ab != "") echo "<a href='" . h(ME . "$z=") . urlencode(is_array($X) ? $X[0] : $X) . "'>$Ab</a> &raquo; ";
																}
															}
															echo "$wg\n";
														}
												...
												{
													static $Vd;
													if (!$Vd) $Vd = base64_encode(rand_string());
													return $Vd;
												}
												function
												page_messages($k)
...
', script("setupSubmitHighlight(document);");
												...
												int32($Pd)
												{
													while ($Pd >= 2147483648) $Pd -= 4294967296;
												...
												$e = '';
												$Jc = $_SESSION["token"];
												if (!$Jc) $_SESSION["token"] = rand(1, 1e6);
												$T = get_token();
												$Ke = array();
												...
												add_invalid_login()
												{
													...
													if ($ad) {
														foreach ($ad
															as $bd => $X) {
															if ($X[0] < $tg) unset($ad[$bd]);
														}
													}
													$Zc = &$ad[$b->bruteForceKey()];
													if (!$Zc) $Zc = array($tg + 30 * 60, 0);
													$Zc[1]++;
												...
												check_invalid_login()
												{
													...
													if ($Ud > 0) auth_error(lang(array('Too many unsuccessful logins, try again in %d minute.', 'Too many unsuccessful logins, try again in %d minutes.'), ceil($Ud / 60)));
												}
												$xa = $_POST["auth"];
												if ($xa) {
													...
													if (count($_POST) == 1 || DRIVER != $dh || SERVER != $O || $_GET["username"] !== $V || DB != $i) redirect(auth_url($dh, $O, $V, $i));
												} elseif ($_POST["logout"]) {
													if ($Jc && !verify_token()) {
													...
													foreach ($Ke
														as $z => $X) {
														list(, $Qa) = explode(":", $X);
														list($dh, $O, $V, $i) = array_map('base64_decode', explode("-", $z));
												...
												unset_permanent()
												{
													...
													foreach ($Ke
														as $z => $X) {
														list($dh, $O, $V, $i) = array_map('base64_decode', explode("-", $z));
														if ($dh == DRIVER && $O == SERVER && $V == $_GET["username"] && $i == DB) unset($Ke[$z]);
													}
													cookie("adminer_permanent", implode(" ", $Ke));
												}
												function
												auth_error($k)
												{
													...
													if (isset($_GET["username"])) {
														...
														if (($_COOKIE[$If] || $_GET[$If]) && !$Jc) $k = 'Session expired, please login again.';
														else {
															add_invalid_login();
															$G = get_password();
															if ($G !== null) {
													...
													if (!$_COOKIE[$If] && $_GET[$If] && ini_bool("session.use_only_cookies")) $k = 'Session support must be enabled.';
												...
												if (isset($_GET["username"])) {
													...
													list($Nc, $Me) = explode(":", SERVER, 2);
													if (is_numeric($Me) && $Me < 1024) auth_error('Connecting to privileged ports is not allowed.');
													check_invalid_login();
													$e = connect();
													$j = new
														Min_Driver($e);
												...
												$wd = null;
												if (!is_object($e) || ($wd = $b->login($_GET["username"], get_password())) !== true) auth_error((is_string($e) ? h($e) : (is_string($wd) ? $wd : 'Invalid credentials.')));
													...
													for ($t = 0; (!$_ || $t < $_) && ($L = $J->fetch_row()); $t++) {
														if (!$t) {
															...
															for ($x = 0; $x < count($L); $x++) {
																...
																$pe = $l->orgname;
																$K[$l->table] = $qe;
																if ($re && $y == "sql") $vd[$x] = ($E == "table" ? "table=" : ($E == "possible_keys" ? "indexes=" : null));
																elseif ($qe != "") {
																	if (!isset($w[$qe])) {
																		$w[$qe] = array();
																...
																$Lg[$x] = $l->type;
																echo "<th" . ($qe != "" || $l->name != $pe ? " title='" . h(($qe != "" ? "$qe." : "") . $pe) . "'" : "") . ">" . h($E) . ($re ? doc_link(array('sql' => "explain-output.html#explain_" . strtolower($E), 'mariadb' => "explain/#the-columns-in-explain-select",)) : "");
															}
															echo "</thead>\n";
														}
														echo "<tr" . odd() . ">";
														foreach ($L
															...
															if (isset($vd[$z]) && !$d[$vd[$z]]) {
																if ($re && $y == "sql") {
																	...
																	foreach ($w[$vd[$z]] as $Ua => $x) $A .= "&where" . urlencode("[" . bracket_escape($Ua) . "]") . "=" . urlencode($L[$x]);
																}
																$X = "<a href='" . h(ME . $A) . "'>$X</a>";
															}
															echo "<td>$X";
												...
												function
												edit_type($z, $l, $Xa, $o = array(), $mc = array())
												{
													global $Yf, $Lg, $Sg, $ge;
													$U = $l["type"];
													echo '<td><select name="', h($z), '[type]" class="type" aria-labelledby="label-type">';
													if ($U && !isset($Lg[$U]) && !isset($o[$U]) && !in_array($U, $mc)) $mc[] = $U;
													if ($o) $Yf['Foreign keys'] = $o;
													echo
...
', on_help("getTarget(event).value", 1), script("mixin(qsl('select'), {onfocus: function () { lastType = selectValue(this); }, onchange: editingTypeChange});", ""), '<td><input name="', h($z), '[length]" value="', h($l["length"]), '" size="3"', (!$l["length"] && preg_match('~var(char|binary)$~', $U) ? " class='required'" : ""), ' aria-labelledby="label-length">', script("mixin(qsl('input'), {onfocus: editingLengthFocus, oninput: editingLengthChange});", ""), '<td class="options">';
												...
												{
													return
														array(idf_escape(trim($l["field"])), process_type($Jg), ($l["null"] ? " NULL" : " NOT NULL"), default_value($l), (preg_match('~timestamp|datetime~', $l["type"]) && $l["on_update"] ? " ON UPDATE $l[on_update]" : ""), (support("comment") && $l["comment"] != "" ? " COMMENT " . q($l["comment"]) : ""), ($l["auto_increment"] ? auto_increment() : null),);
												}
												function
												default_value($l)
												{
...
';
												...
												process_fields(&$m)
												{
													...
													if ($_POST["up"]) {
														...
														foreach ($m
															...
															if (key($_POST["up"]) == $z) {
																unset($m[$z]);
																array_splice($m, $md, 0, array($l));
																break;
															}
															if (isset($l["field"])) $md = $Zd;
															$Zd++;
														}
													} elseif ($_POST["down"]) {
														$zc = false;
														foreach ($m
															...
															if (isset($l["field"]) && $zc) {
																...
																array_splice($m, $Zd, 0, array($zc));
																break;
															}
															if (key($_POST["down"]) == $z) $zc = $l;
															$Zd++;
														}
													} elseif ($_POST["add"]) {
														$m = array_values($m);
												...
												grant($r, $Xe, $d, $fe)
												{
													if (!$Xe) return
														true;
													if ($Xe == array("ALL PRIVILEGES", "GRANT OPTION")) return ($r == "GRANT" ? queries("$r ALL PRIVILEGES$fe WITH GRANT OPTION") : queries("$r ALL PRIVILEGES$fe") && queries("$r GRANT OPTION$fe"));
													return
														queries("$r " . preg_replace('~(GRANT OPTION)\\([^)]*\\)~', '\\1', implode("$d, ", $Xe) . $d) . $fe);
												}
												function
												drop_create($Jb, $g, $Kb, $qg, $Lb, $B, $Jd, $Hd, $Id, $ce, $Sd)
												{
													if ($_POST["drop"]) query_redirect($Jb, $B, $Jd);
													elseif ($ce == "") query_redirect($g, $B, $Id);
													elseif ($ce != $Sd) {
														$lb = queries($g);
												...
												create_routine($uf, $L)
												{
													...
													foreach ($m
														as $l) {
														if ($l["field"] != "") $P[] = (preg_match("~^($Vc)\$~", $l["inout"]) ? "$l[inout] " : "") . idf_escape($l["field"]) . process_type($l, "CHARACTER SET");
													}
													$zb = rtrim("\n$L[definition]", ";");
												...
												function
												doc_link($Ie, $rg = "<sup>?</sup>")
												{
													global $y, $e;
													$Gf = $e->server_info;
													$eh = preg_replace('~^(\\d\\.?\\d).*~s', '\\1', $Gf);
													$Wg = array('sql' => "https://dev.mysql.com/doc/refman/$eh/en/", 'sqlite' => "https://www.sqlite.org/", 'pgsql' => "https://www.postgresql.org/docs/$eh/static/", 'mssql' => "https://msdn.microsoft.com/library/", 'oracle' => "https://download.oracle.com/docs/cd/B19306_01/server.102/b14200/",);
												...
												if (isset($_GET["download"])) {
													...
													foreach (table_status('', true) as $Q => $R) {
														...
														$zf[$Q]["pos"] = ($hg[$Q] ? $hg[$Q] : array($Bg, 0));
														foreach ($b->foreignKeys($Q) as $X) {
															if (!$X["db"]) {
																$od = $Ca;
																if ($hg[$Q][1] || $hg[$X["table"]][1]) $od = min(floatval($hg[$Q][1]), floatval($hg[$X["table"]][1])) - 1;
																else $Ca -= .1;
																while ($qd[(string)$od]) $od -= .0001;
																$zf[$Q]["references"][$X["table"]][(string)$od] = array($X["source"], $X["target"]);
...
';
													...
													if ($_POST && !$k) {
														$jb = "";
														foreach (array("output", "format", "db_style", "routines", "events", "table_style", "auto_increment", "triggers", "data_style") as $z) $jb .= "&$z=" . urlencode($_POST[$z]);
														...
														foreach ((array)$h
															...
															if ($e->select_db($i)) {
																if ($dd && preg_match('~CREATE~', $Zf) && ($g = $e->result("SHOW CREATE DATABASE " . idf_escape($i), 1))) {
																	set_utf8mb4($g);
																	if ($Zf == "DROP+CREATE") echo "DROP DATABASE IF EXISTS " . idf_escape($i) . ";\n";
																	echo "$g;\n";
																}
																if ($dd) {
																	if ($Zf) echo
																	...
																	$xe = "";
																	if ($_POST["routines"]) {
																		foreach (array("FUNCTION", "PROCEDURE") as $uf) {
																...
																if ($_POST["table_style"] || $_POST["data_style"]) {
																	...
																	foreach (table_status('', true) as $E => $R) {
																		...
																		if ($Q || $rb) {
																			...
																			if ($dd && $_POST["triggers"] && $Q && ($Ig = trigger_sql($E))) echo "\nDELIMITER ;;\n$Ig\nDELIMITER ;\n";
																			if ($kc == "tar") {
																				ob_end_flush();
																	...
																	foreach ($gh
																		as $fh) $b->dumpTable($fh, $_POST["table_style"], 1);
																	if ($kc == "tar") echo
																	pack("x512");
																}
															}
														}
														if ($dd) echo "-- " . $e->result("SELECT NOW()") . "\n";
														exit;
													}
...
', script("qsl('table').onclick = dumpClick;");
													...
													if (DB != "") {
														...
														foreach ($kg
															...
															if ($U !== null && !preg_match('~table~i', $U)) $gh .= "$Ue\n";
															else
...
';
													...
													if (!$k && $_POST) {
														...
														if (is_string($I)) {
															...
															$Wb = true;
															$f = connect();
															if (is_object($f) && DB != "") $f->select_db(DB);
															$ab = 0;
															$bc = array();
																	...
																	if (!$zc && $p && !feof($p)) $I .= fread($p, 1e5);
																		...
																		if ($zc && rtrim($zc) != $_b) {
																			...
																			if ($y == "sqlite" && preg_match("~^$Of*+ATTACH\\b~i", $H, $C)) {
																				...
																				$Tf = microtime(true);
																				if ($e->multi_query($H) && is_object($f) && preg_match("~^$Of*+USE\\b~i", $H)) $f->query($H);
																				do {
																					$J = $e->store_result();
																					if ($e->error) {
																						...
																						$kh = "warnings-$ab";
																						if ($jh) $tg .= ", <a href='#$kh'>" . 'Warnings' . "</a>" . script("qsl('a').onclick = partial(toggle, '$kh');", "");
																						$ic = null;
																						$jc = "explain-$ab";
																						if (is_object($J)) {
																							$_ = $_POST["limit"];
																							$re = select($J, $f, array(), $_);
																							if (!$_POST["only_errors"]) {
																								echo "<form action='' method='post'>\n";
																								$Wd = $J->num_rows;
																								echo "<p>" . ($Wd ? ($_ && $Wd > $_ ? sprintf('%d / ', $_) : "") . lang(array('%d row', '%d rows'), $Wd) : ""), $tg;
																								if ($f && preg_match("~^($Of|\\()*+SELECT\\b~i", $H) && ($ic = explain($f, $H))) echo ", <a href='#$jc'>Explain</a>" . script("qsl('a').onclick = partial(toggle, '$jc');", "");
																								$u = "export-$ab";
																							...
																							if (!$_POST["only_errors"]) echo "<p class='message' title='" . h($e->info) . "'>" . lang(array('Query executed OK, %d row affected.', 'Query executed OK, %d rows affected.'), $na) . "$tg\n";
																						}
															...
															elseif ($_POST["only_errors"]) {
																echo "<p class='message'>" . lang(array('%d query executed OK.', '%d queries executed OK.'), $ab - count($bc)), " <span class='time'>(" . format_time($Cg) . ")</span>\n";
...
';
													...
													if (!isset($_GET["import"])) {
														...
														elseif ($_GET["history"] == "all") $H = $Lc;
														elseif ($_GET["history"] != "") $H = $Lc[$_GET["history"]][0];
														echo "<p>";
														textarea("query", $H, 20);
...
';
													...
													foreach ($m
														as $E => $l) {
														if (!isset($l["privileges"][$Tg ? "update" : "insert"]) || $b->fieldName($l) == "") unset($m[$E]);
													}
													if ($_POST && !$k && !isset($_GET["select"])) {
														$B = $_POST["referer"];
														if ($_POST["insert"]) $B = ($Tg ? null : $_SERVER["REQUEST_URI"]);
														elseif (!preg_match('~^.+&select=.+$~', $B)) $B = ME . "select=" . urlencode($a);
														$w = indexes($a);
														$Og = unique_array($_GET["where"], $w);
														$df = "\nWHERE $Z";
														if (isset($_POST["delete"])) queries_redirect($B, 'Item has been deleted.', $j->delete($a, $df, !$Og));
														else {
															$P = array();
															foreach ($m
																as $E => $l) {
													...
													$a = $_GET["create"];
													$De = array();
													foreach (array('HASH', 'LINEAR HASH', 'KEY', 'LINEAR KEY', 'RANGE', 'LIST') as $z) $De[$z] = $z;
													$jf = referencable_primary($a);
													$o = array();
													foreach ($jf
														as $gg => $l) $o[str_replace("`", "``", $gg) . "`" . str_replace("`", "``", $l["field"])] = $gg;
													$ue = array();
													$R = array();
													if ($a != "") {
														$ue = fields($a);
														...
														if ($_POST["drop"]) queries_redirect(substr(ME, 0, -1), 'Table has been dropped.', drop_tables(array($a)));
															...
															foreach ($L["fields"] as $z => $l) {
																...
																if ($l["field"] != "") {
																	...
																	$ra[] = array($l["orig"], $Ze, $pa);
																	if ($Ze != process_field($te, $te)) {
																		$m[] = array($l["orig"], $Ze, $pa);
																		if ($l["orig"] != "" || $pa) $Xg = true;
																	}
																	if ($n !== null) $wc[idf_escape($l["field"])] = ($a != "" && $y != "sqlite" ? "ADD" : " ") . format_foreign_key(array('table' => $o[$l["type"]], 'source' => array($l["field"]), 'target' => array($Jg["field"]), 'on_delete' => $l["on_delete"],));
															...
															} elseif (support("partitioning") && preg_match("~partitioned~", $R["Create_options"])) $Fe .= "\nREMOVE PARTITIONING";
													...
													if (!$_POST) {
														$L = array("Engine" => $_COOKIE["adminer_engine"], "fields" => array(array("field" => "", "type" => (isset($Lg["int"]) ? "int" : (isset($Lg["integer"]) ? "integer" : "")), "on_update" => "")), "partition_names" => array(""),);
														if ($a != "") {
															$L = $R;
															$L["name"] = $a;
															$L["fields"] = array();
...
';
														...
														echo ($Yb ? "<select name='Engine'>" . optionlist(array("" => "(" . 'engine' . ")") + $Yb, $L["Engine"]) . "</select>" . on_help("getTarget(event).value", 1) . script("qsl('select').onchange = helpClose;") : ""), ' ', ($Xa && !preg_match("~sqlite|mssql~", $y) ? html_select("Collation", array("" => "(" . 'collation' . ")") + $Xa, $L["Collation"]) : ""), ' <input type="submit" value="Save">
';
													}
													echo '
';
...
';
													...
													if (support("partitioning")) {
														...
														echo '<p>
', "<select name='partition_by'>" . optionlist(array("" => "") + $De, $L["partition_by"]) . "</select>" . on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')", 1) . script("qsl('select').onchange = partitionByChange;"), '(<input name="partition" value="', h($L["partition"]), '">)
...
', script("qs('#form')['defaults'].onclick();" . (support("comment") ? " editingCommentsClick.call(qs('#form')['comments']);" : ""));
													...
													$a = $_GET["indexes"];
													$Tc = array("PRIMARY", "UNIQUE", "INDEX");
													$R = table_status($a, true);
													if (preg_match('~MyISAM|M?aria' . (min_version(5.6, '10.0.5') ? '|InnoDB' : '') . '~i', $R["Engine"])) $Tc[] = "FULLTEXT";
													if (preg_match('~MyISAM|M?aria' . (min_version(5.7, '10.2.2') ? '|InnoDB' : '') . '~i', $R["Engine"])) $Tc[] = "SPATIAL";
													$w = indexes($a);
													$Te = array();
													if ($y == "mongo") {
														$Te = $w["_id_"];
													...
													if ($_POST && !$k && !$_POST["add"] && !$_POST["drop_col"]) {
														...
														foreach ($w
															as $E => $hc) $sa[] = array($hc["type"], $E, "DROP");
					...
					<?php
													...
													foreach ($L["indexes"] as $v) {
														if (!$_POST["drop_col"] || $x != key($_POST["drop_col"])) {
															echo "<tr><td>" . html_select("indexes[$x][type]", array(-1 => "") + $Tc, $v["type"], ($x == count($L["indexes"]) ? "indexesAddRow.call(this);" : 1), "label-type"), "<td>";
															ksort($v["columns"]);
															$t = 1;
															foreach ($v["columns"] as $z => $c) {
														...
														if ($_POST["drop"]) {
															...
															if (DB != "") {
																...
																foreach ($h
																	as $i) {
																	if (count($h) == 1 || $i != "") {
																		if (!create_database($i, $L["collation"])) $ag = false;
																		$md = $i;
																	}
																}
																restart_session();
...
<form action="" method="post">
<p>
', ($_POST["add_x"] || strpos($E, "\n") ? '<textarea id="name" name="name" rows="10" cols="40">' . h($E) . '</textarea><br>' : '<input name="name" id="name" value="' . h($E) . '" maxlength="64" autocapitalize="off">') . "\n" . ($Xa ? html_select("collation", array("" => "(" . 'collation' . ")") + $Xa, $L["collation"]) . doc_link(array('sql' => "charset-charsets.html", 'mariadb' => "supported-character-sets-and-collations/", 'mssql' => "ms187963.aspx",)) : ""), script("focus(qs('#name'));"), '<input type="submit" value="Save">
';
													if (DB != "") echo "<input type='submit' name='drop' value='" . 'Drop' . "'>" . confirm(sprintf('Drop %s?', DB)) . "\n";
													elseif (!$_POST["add_x"] && $_GET["db"] == "") echo "<input type='image' class='icon' name='add' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=plus.gif&version=4.6.2&driver=mysql") . "' alt='+' title='" . 'Add next' . "'>\n";
													echo '<input type="hidden" name="token" value="', $T, '">
</form>
';
													...
													foreach ($uf["fields"] as $t => $l) {
														if (substr($l["inout"], -3) == "OUT") $xe[$t] = "@" . idf_escape($l["field"]) . " AS " . idf_escape($l["field"]);
														if (!$l["inout"] || substr($l["inout"], 0, 2) == "IN") $Sc[] = $t;
													}
													if (!$k && $_POST) {
														$Ja = array();
														foreach ($uf["fields"] as $z => $l) {
															if (in_array($z, $Sc)) {
																$X = process_input($l);
																if ($X === false) $X = "''";
																if (isset($xe[$z])) $e->query("SET @" . idf_escape($l["field"]) . " = $X");
															}
														...
														if (!$J) echo "<p class='error'>" . error() . "\n";
															...
															if ($xe) select($e->query("SELECT " . implode(", ", $xe)));
														}
													}
													echo '
<form action="" method="post">
...
';
													...
													if ($_POST && !$k && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
														...
														if (!$_POST["drop"]) {
															...
															ksort($L["source"]);
															$ng = array();
															foreach ($L["source"] as $z => $X) $ng[$z] = $L["target"][$z];
															$L["target"] = $ng;
														}
...
';
														...
														foreach ($L["source"] as $z => $X) {
															echo "<tr>", "<td>" . html_select("source[" . (+$z) . "]", array(-1 => "") + $Nf, $X, ($x == count($L["source"]) - 1 ? "foreignAddRow.call(this);" : 1), "label-source"), "<td>" . html_select("target[" . (+$z) . "]", $ng, $L["target"][$z], 1, "label-target");
															$x++;
														}
														echo '</table>
<p>
ON DELETE: ', html_select("on_delete", array(-1 => "") + explode("|", $ge), $L["on_delete"]), ' ON UPDATE: ', html_select("on_update", array(-1 => "") + explode("|", $ge), $L["on_update"]), doc_link(array('sql' => "innodb-foreign-key-constraints.html", 'mariadb' => "foreign-keys/", 'pgsql' => "sql-createtable.html#SQL-CREATETABLE-REFERENCES", 'mssql' => "ms174979.aspx", 'oracle' => "clauses002.htm#sthref2903",)), '<p>
<input type="submit" value="Save">
...
';
													...
													if ($_POST && !$k) {
														...
														if (!$_POST["drop"] && $a == $E && $y != "sqlite" && $U == "VIEW" && $ve == "VIEW") query_redirect(($y == "mssql" ? "ALTER" : "CREATE OR REPLACE") . " VIEW " . table($E) . $ua, $B, $D);
														else {
...
';
													...
													$aa = $_GET["event"];
													$Yc = array("YEAR", "QUARTER", "MONTH", "DAY", "HOUR", "MINUTE", "WEEK", "SECOND", "YEAR_MONTH", "DAY_HOUR", "DAY_MINUTE", "DAY_SECOND", "HOUR_MINUTE", "HOUR_SECOND", "MINUTE_SECOND");
													$Vf = array("ENABLED" => "ENABLE", "DISABLED" => "DISABLE", "SLAVESIDE_DISABLED" => "DISABLE ON SLAVE");
													$L = $_POST;
													if ($_POST && !$k) {
...
';
													...
													if ($_POST && !process_fields($L["fields"]) && !$k) {
														...
														drop_create("DROP $uf " . routine_id($da, $se), create_routine($uf, $L), "DROP $uf " . routine_id($L["name"], $L), create_routine($uf, array("name" => $pg) + $L), "DROP $uf " . routine_id($pg, $L), substr(ME, 0, -1), 'Routine has been dropped.', 'Routine has been altered.', 'Routine has been created.', $da, $L["name"]);
													}
													...
													if ($_POST) {
														if (!$k && in_array($_POST["Timing"], $Hg["Timing"]) && in_array($_POST["Event"], $Hg["Event"]) && in_array($_POST["Type"], $Hg["Type"])) {
															...
															if ($_POST["drop"]) query_redirect($Jb, $B, 'Trigger has been dropped.');
																...
																if ($E != "") queries(create_trigger($fe, $L + array("Type" => reset($Hg["Type"]))));
															}
														}
														$L = $_POST;
													}
...
';
													...
													$fa = $_GET["user"];
													$Xe = array("" => array("All privileges" => ""));
													foreach (get_rows("SHOW PRIVILEGES") as $L) {
														foreach (explode(",", ($L["Privilege"] == "Grant option" ? "" : $L["Context"])) as $hb) $Xe[$hb][$L["Privilege"]] = $L["Comment"];
													}
													...
													$Xe["Columns"] = array();
													foreach (array("Select", "Insert", "Update", "References") as $X) $Xe["Columns"][$X] = $Xe["Tables"][$X];
													unset($Xe["Server Admin"]["Usage"]);
													foreach ($Xe["Tables"] as $z => $X) unset($Xe["Databases"][$z]);
													$Rd = array();
													if ($_POST) {
														foreach ($_POST["objects"] as $z => $X) $Rd[$X] = (array)$Rd[$X] + (array)$_POST["grants"][$z];
													}
													$Cc = array();
													$de = "";
													if (isset($_GET["host"]) && ($J = $e->query("SHOW GRANTS FOR " . q($fa) . "@" . q($_GET["host"])))) {
														while ($L = $J->fetch_row()) {
															if (preg_match('~GRANT (.*) ON (.*) TO ~', $L[0], $C) && preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~', $C[1], $_d, PREG_SET_ORDER)) {
																foreach ($_d
																	as $X) {
																	if ($X[1] != "USAGE") $Cc["$C[2]$X[2]"][$X[1]] = true;
																	if (preg_match('~ WITH GRANT OPTION~', $L[0])) $Cc["$C[2]$X[2]"]["GRANT OPTION"] = true;
																}
															}
															if (preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~", $L[0], $C)) $de = $C[1];
														}
													}
													if ($_POST && !$k) {
														$ee = (isset($_GET["host"]) ? q($fa) . "@" . q($_GET["host"]) : "''");
														if ($_POST["drop"]) query_redirect("DROP USER $ee", ME . "privileges=", 'User has been dropped.');
														else {
															...
															if (!$k) {
																if ($ee != $Td) {
																	...
																	$k = !$lb;
																} elseif ($He != $de) queries("SET PASSWORD FOR $Td = " . q($He));
															}
															if (!$k) {
																$rf = array();
																foreach ($Rd
																	...
																	$r = array_keys($r);
																	if (isset($_GET["grant"])) $rf = array_diff(array_keys(array_filter($Rd[$Yd], 'strlen')), $r);
																	elseif ($ee == $Td) {
															...
															if (!$k && isset($_GET["host"])) {
																if ($ee != $Td) queries("DROP USER $ee");
																elseif (!isset($_GET["grant"])) {
																	foreach ($Cc
																		as $Yd => $rf) {
																		if (preg_match('~^(.+)(\\(.*\\))?$~U', $Yd, $C)) grant("REVOKE", array_keys($rf), $C[2], " ON $C[1] FROM $Td");
																	}
																}
															}
															queries_redirect(ME . "privileges=", (isset($_GET["host"]) ? 'User has been altered.' : 'User has been created.'), !$k);
															if ($lb) $e->query("DROP USER $Td");
														}
													}
													...
													if ($_POST) {
														...
														$L["pass"] = $de;
														if ($de != "") $L["hashed"] = true;
														$Cc[(DB == "" || $Cc ? "" : idf_escape(addcslashes(DB, "%_\\"))) . ".*"] = array();
													}
													echo '<form action="" method="post">
<table cellspacing="0">
...
';
													...
													echo "</thead>\n";
													foreach (array("" => "", "Server Admin" => 'Server', "Databases" => 'Database', "Tables" => 'Table', "Columns" => 'Column', "Procedures" => 'Routine',) as $hb => $Ab) {
														foreach ((array)$Xe[$hb] as $We => $bb) {
															...
															foreach ($Cc
																...
																$Y = $r[strtoupper($We)];
																if ($hb == "Server Admin" && $Yd != (isset($Cc["*.*"]) ? "*.*" : ".*")) echo "<td>&nbsp;";
...
';
													...
													if (support("kill") && $_POST && !$k) {
														...
														foreach ((array)$_POST["kill"] as $X) {
															if (kill_process($X)) $id++;
														}
														queries_redirect(ME . "processlist=", lang(array('%d process has been killed.', '%d processes have been killed.'), $id), $id || !$_POST["kill"]);
													}
...
', script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});");
													...
													foreach (process_list() as $t => $L) {
														...
														foreach ($L
															as $z => $X) echo "<td>" . (($y == "sql" && $z == "Info" && preg_match("~Query|Killed~", $L["Command"]) && $X != "") || ($y == "pgsql" && $z == "current_query" && $X != "<IDLE>") || ($y == "oracle" && $z == "sql_text" && $X != "") ? "<code class='jush-$y'>" . shorten_utf8($X, 100, "</code>") . ' <a href="' . h(ME . ($L["db"] != "" ? "db=" . urlencode($L["db"]) . "&" : "") . "sql=" . urlencode($X)) . '">' . 'Clone' . '</a>' : nbsp($X));
														echo "\n";
													}
													echo '</table>
<p>
...
', script("tableCheck();");
													...
													foreach ($m
														...
														if (isset($l["privileges"]["select"]) && $E != "") {
															...
															if (is_shortable($l)) $sg = $b->selectLengthProcess();
														}
														$sf += $l["privileges"];
													}
													...
													foreach ($w
														...
														if ($v["type"] == "PRIMARY") {
															...
															foreach ($Rg
																as $z => $X) {
																if (in_array(idf_escape($z), $N)) unset($Rg[$z]);
															}
															break;
														}
													}
													if ($ae && !$Te) {
														$Te = $Rg = array($ae => 0);
														$w[] = array("type" => "PRIMARY", "columns" => array($ae));
													}
													if ($_POST && !$k) {
														$mh = $Z;
														...
														if ($_POST["export"]) {
															...
															$Ec = ($s && $cd ? "\nGROUP BY " . implode(", ", $s) : "") . ($oe ? "\nORDER BY " . implode(", ", $oe) : "");
															if (!is_array($_POST["check"]) || $Te) $I = "SELECT $Ac$mh$Ec";
															else {
																$Ng = array();
																foreach ($_POST["check"] as $X) $Ng[] = "(SELECT" . limit($Ac, "\nWHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($X, $m) . $Ec, 1) . ")";
														...
														if (!$b->selectEmailProcess($Z, $o)) {
															if ($_POST["save"] || $_POST["delete"]) {
																...
																if ($_POST["clone"] && $J && $na == 1) {
																	$nd = last_id();
																	if ($nd) $D = sprintf('Item%s has been inserted.', " $nd");
																}
															...
															elseif (!preg_match('~~u', $rc)) $k = 'File must be in UTF-8 encoding.';
																...
																foreach ($_d[0] as $z => $X) {
																	...
																	if (!$z && !array_diff($Ad[1], $Ya)) {
																		...
																		$P = array();
																		foreach ($Ad[1] as $t => $Ua) $P[idf_escape($Ya[$t])] = ($Ua == "" && $m[$Ya[$t]]["null"] ? "NULL" : q(str_replace('""', '"', preg_replace('~^"|"$~', '', $Ua))));
																		$M[] = $P;
																	}
																}
													...
													if (!$d && support("table")) echo "<p class='error'>" . 'Unable to select the table' . ($m ? "." : ": " . error()) . "\n";
														...
														if (!$Af) {
															...
															if ($ib) $Af[] = substr($ib, 2);
														}
														foreach ($N
															as $z => $X) {
															$l = $m[idf_unescape($X)];
															if ($l && ($ua = convert_field($l))) $Af[$z] = "$ua AS $X";
														}
														if (!$cd && $Rg) {
															foreach ($Rg
																as $z => $X) {
																$Af[] = idf_escape($z);
																if ($Dc) $Dc[] = idf_escape($z);
															}
														}
														...
														if (!$J) echo "<p class='error'>" . error() . "\n";
															...
															if ($_GET["page"] != "last" && $_ != "" && $s && $cd && $y == "sql") $_c = $e->result(" SELECT FOUND_ROWS()");
															if (!$M) echo "<p class='message'>" . 'No rows.' . "\n";
																...
																if ($_GET["modify"]) {
																	foreach ($M
																		as $L) {
																		foreach ($L
																			as $z => $X) $td[$z] = max($td[$z], min(40, strlen(utf8_decode($X))));
																	}
																}
																...
																foreach ($b->rowDescriptions($M, $o) as $Pd => $L) {
																	...
																	if (!$Og) {
																		$Og = array();
																		foreach ($M[$Pd] as $z => $X) {
																			if (!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~', $z)) $Og[$z] = $X;
																		}
																	}
																	$Pg = "";
																	foreach ($Og
																	...
																	foreach ($L
																		...
																		if (isset($Qd[$z])) {
																			$l = $m[$z];
																			$X = $j->value($X, $l);
																			if ($X != "" && (!isset($Vb[$z]) || $Vb[$z] != "")) $Vb[$z] = (is_mail($X) ? $Qd[$z] : "");
																			$A = "";
																			if (preg_match('~blob|bytea|raw|file~', $l["type"]) && $X != "") $A = ME . 'download=' . urlencode($a) . '&field=' . urlencode($z) . $Pg;
																			if (!$A && $X !== null) {
																				foreach ((array)$o[$z] as $n) {
																					if (count($o[$z]) == 1 || end($n["source"]) == $z) {
																						$A = "";
																						foreach ($n["source"] as $t => $Nf) $A .= where_link($t, $n["target"][$t], $M[$Pd][$Nf]);
																			...
																			if ($z == "COUNT(*)") {
																				...
																				foreach ((array)$_GET["where"] as $W) {
																					if (!array_key_exists($W["col"], $Og)) $A .= where_link($t++, $W["col"], $W["val"], $W["op"]);
																				}
																				foreach ($Og
																					as $fd => $W) $A .= where_link($t++, $fd, $W);
																			}
															...
															if (!is_ajax()) {
																if ($M || $F) {
																	$fc = true;
																	if ($_GET["page"] != "last") {
																		if ($_ == "" || (count($M) < $_ && ($M || !$F))) $_c = ($F ? $F * $_ : 0) + count($M);
																		elseif ($y != "sql" || !$cd) {
																			$_c = ($cd ? false : found_rows($R, $Z));
																			if ($_c < max(1e4, 2 * ($F + 1) * $_)) $_c = reset(slow_query(count_rows($a, $Z, $cd, $s)));
																			else $fc = false;
																		}
																	}
																...
																if ($M || $F) {
																	if ($_e) {
																		...
																		if ($y != "simpledb") {
																			...
																			for ($t = max(1, $F - 4); $t < min($Cd, $F + 5); $t++) echo
																			pagination($t, $F);
																			if ($Cd > 0) {
																	...
																	$Fb = ($fc ? "" : "~ ") . $_c;
																	echo
																	checkbox("all", 1, 0, ($_c !== false ? ($fc ? "" : "~ ") . lang(array('%d row', '%d rows'), $_c) : ""), "var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Fb' : checked); selectCount('selected2', this.checked || !checked ? '$Fb' : checked);") . "\n", "</fieldset>\n";
...
';
													...
													if (!$ch) echo "<p class='message'>" . 'No rows.' . "\n";
													else {
														echo "<table cellspacing='0'>\n";
														foreach ($ch
															as $z => $X) {
													...
													header("Content-Type: text/javascript; charset=utf-8");
													if ($_GET["script"] == "db") {
														$dg = array("Data_length" => 0, "Index_length" => 0, "Data_free" => 0);
														foreach (table_status() as $E => $R) {
															json_row("Comment-$E", nbsp($R["Comment"]));
															if (!is_view($R)) {
																foreach (array("Engine", "Collation") as $z) json_row("$z-$E", nbsp($R[$z]));
																foreach ($dg + array("Auto_increment" => 0, "Rows" => 0) as $z => $X) {
																	if ($R[$z] != "") {
																		$X = format_number($R[$z]);
																		json_row("$z-$E", ($z == "Rows" && $X && $R["Engine"] == ($Pf == "pgsql" ? "table" : "InnoDB") ? "~ $X" : $X));
																		if (isset($dg[$z])) $dg[$z] += ($R["Engine"] != "InnoDB" || $z != "Data_free" ? $R[$z] : 0);
													...
													if ($b->homepage()) {
														if ($_GET["ns"] !== "") {
															...
															$kg = tables_list();
															if (!$kg) echo "<p class='message'>" . 'No tables.' . "\n";
															else {
																echo "<form action='' method='post'>\n";
																if (support("table")) {
